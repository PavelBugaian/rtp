defmodule Transmitter.WorkerSupervisor do
  use GenServer
  ...
  def init(:ok) do
    ...
    Transmitter.WorkerService.set_workers(Map.values(refs))
    {:ok, refs}
  end
  ...
  def add_process(refs, count) do
    case count do
      0 -> refs
      _ ->
        {:ok, pid} = DynamicSupervisor.start_child(WorkerDynamicSupervisor, Transmitter.Worker)
        ref = Process.monitor(pid)
        refs = Map.put(refs, ref, pid)
        Transmitter.WorkerSupervisor.add_process(refs, count - 1)
    end
  end
